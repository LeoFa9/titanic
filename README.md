...
финальный пайплайн) - `python run_notebooks.py`. Долго (суммарно может уйти больше часа - CatBoost
и DNN грид-серчи в classic_models_exploration.ipynb/DNN_exploration.ipynb сами по себе занимают
десятки минут), печатает `OK`/`FAILED` по каждому ноутбуку.
